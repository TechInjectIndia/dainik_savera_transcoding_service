...
const DB_NAME = 'dainik_savera';
const DB_USER = 'sumeet';
const DB_PASS = 'root'; // Use environment variables in production!
...
tusServer.on(EVENTS.POST_CREATE, async (event: TusEvent) => {
	...
	if (typeof locationHeader !== 'string' || !locationHeader) {
		console.warn(`[EVENT:POST_CREATE] Could not extract Location header from response.`); return;
	}
	const urlParts = locationHeader.split(tusPath);
	const uploadId = urlParts[1];
	if (!uploadId) {
		console.warn(`[EVENT:POST_CREATE] Could not parse upload ID from Location header: ${locationHeader}`); return;
	}

	const metadata = parseMetadata(metadataHeader as string | undefined);
	...
	if (originalFilename) {
		try {
			...
			if (created) { console.log(`[EVENT:POST_CREATE] Saved metadata to DB for ID: ${uploadId}, Filename: ${originalFilename}`); }
			else { console.warn(`[EVENT:POST_CREATE] Metadata record already existed for ID: ${uploadId}.`); }
		} catch (dbError) { console.error(`[EVENT:POST_CREATE] Error saving metadata to DB for ID ${uploadId}:`, dbError); }
	} else { console.warn(`[EVENT:POST_CREATE] Could not save metadata. Original filename ('name') missing in Upload-Metadata header for ID: ${uploadId}.`); }
});
	...
	try {
		...
		if (originalFilename) {
			...
			if (sanitizedFilename) {
				...
				} catch (renameError: any) { console.error(`[Rename] Error during rename process:`, renameError); return; }
			} else { console.warn(`[Rename] Original filename "${originalFilename}" sanitized to an empty string. File will not be renamed.`); }
		} else { console.log(`[Rename] No metadata record found in DB for ID ${uploadId}. File will not be renamed.`); }
	} catch (dbError) { console.error(`[Rename] Error querying DB for ID ${uploadId}:`, dbError); return;
	} finally {
		if (uploadRecord) {
			try { await uploadRecord.destroy(); console.log(`[Rename] Removed metadata record from DB for ID ${uploadId}.`); }
			catch (deleteError) { console.error(`[Rename] Error deleting metadata record from DB for ID ${uploadId}:`, deleteError); }
		} else { console.warn(`[Rename] Could not find metadata record for ID ${uploadId} in DB to remove.`); }
	}
});
...
tusServer.on(EVENTS.POST_TERMINATE, async (event: TusEvent) => {
	...
	else if (event.url) { const urlParts = event.url.split(tusPath); uploadId = urlParts[1]; }
	if (uploadId) {
		...
		try {
			...
			if (deletedCount > 0) { console.log(`[EVENT:POST_TERMINATE] Cleaned up metadata from DB for terminated upload ID ${uploadId}.`); }
			else { console.log(`[EVENT:POST_TERMINATE] No metadata found in DB to cleanup for terminated upload ID ${uploadId}.`); }
		} catch (deleteError) { console.error(`[EVENT:POST_TERMINATE] Error cleaning up metadata from DB for terminated upload ID ${uploadId}:`, deleteError); }
